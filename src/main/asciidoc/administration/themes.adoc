...
This displays a list of all the theme objects based on parameters like page, size or sort feature based on name, organization etc as shown in *Query Parameters*.

===== Query Parameters
include::../{snippets}/themes/directory/query-parameters.adoc[]
...
A theme is created by adding a fields like _name_, _organization_, _header_, _footer_ etc along with other styling variables as shown in the *Query Parameters*. A theme when created, by default, has the _active_ check set to false. This boolean flag, when checked will activate the specific theme.
...
===== Query Parameters
include::../{snippets}/themes/patch/query-parameters.adoc[]
